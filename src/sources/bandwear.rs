...
impl AlertSource for Bandwear {
    ...
    fn fetch(&self) -> Result<Vec<AlertData>, AlertyError> {
        ...
        let res = ureq::get(&format!(
            "https://shop.bandwear.com/collections/{shop_name}?sort_by=created-descending"
        ))
        .call();
        let res = match res {
            Ok(res) => res,
        ...
        let thumb_selector =
            Selector::parse("div.thumbnail > a > div.product_image > img").unwrap();
        let title_selector = Selector::parse("div.thumbnail > a > div.info > span.title").unwrap();
        let price_selector =
            Selector::parse("div.thumbnail > a > div.info > span.price > span").unwrap();
        ...
        .map(|(link, thumb, title, price)| {
            let link = link
                .attr("href")
                .map(|link| format!("https://shop.bandwear.com/{link}"));
